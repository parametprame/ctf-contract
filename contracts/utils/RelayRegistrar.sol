...
contract RelayRegistrar is IRelayRegistrar, Ownable, ERC165 {
    ...
    /// @inheritdoc IRelayRegistrar
    function getRelayRegistrationMaxAge()
        external
        view
        override
        returns (uint256)
    ...
    /// @inheritdoc IRelayRegistrar
    function setRelayRegistrationMaxAge(
        uint256 _relayRegistrationMaxAge
    ...
    function supportsInterface(
        ...
        return
            interfaceId == type(IRelayRegistrar).interfaceId ||
    ...
    ) internal returns (RelayInfo storage) {
    ...
    function getRelayInfo(
        address relayHub,
        address relayManager
    ) public view override returns (RelayInfo memory) {
    ...
    function readRelayInfos(
        ...
        uint256 oldestBlockTimestamp = blockTimestamp >= relayRegistrationMaxAge
    ...
    function readRelayInfosInRange(
        ...
        for (uint256 i = 0; i < items.length; i++) {
            ...
            try
                IRelayHub(relayHub).verifyRelayManagerStaked(relayManager)
